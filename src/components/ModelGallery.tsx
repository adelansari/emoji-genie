...
import { mouthModels, MouthShapeType, MouthModel } from "../data/mouthModels";
...
type ModelGalleryProps = {
  ...
  currentMouthModel: MouthShapeType | null;
};
...
export default function ModelGallery({
  ...
  const renderMouthModels = () => {
    return mouthModels.map((model: MouthModel) => {
      if (!model.SvgComponent) return null;

      const fillColor = currentMouthModel === model.id ? '#F87171' : '#D1D5DB'; // Example: Red when selected
      const borderClass = currentMouthModel === model.id
      ...
      return (
        <button
          key={`Mouth-${model.id}`}
          onClick={() => onSelectModel("Mouth", model.id)}
          className={`flex-shrink-0 w-16 h-16 flex items-center justify-center p-1 rounded border-2 transition-colors duration-150 ${borderClass}`}
          title={model.name}
        >
          <model.SvgComponent
